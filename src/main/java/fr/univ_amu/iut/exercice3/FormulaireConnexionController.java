...
public class FormulaireConnexionController {
  ...
  private void initialize() {
    ...
    boutonAnnuler
        ...
        .bind(
            Bindings.and(
                Bindings.equal(champIdentifiant.textProperty().length(), 0),
                Bindings.equal(champMotDePasse.textProperty().length(), 0)));
  ...
  private void valider() {
    ...
    String indentifiant = champIdentifiant.getText();
    String masque = "*".repeat(champMotDePasse.getText().length());
    labelMessage.setText(indentifiant + " " + masque);
  }
